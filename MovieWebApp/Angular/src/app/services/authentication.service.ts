...
export class AuthenticationService {
  ...
  register(user:Credentials):Promise<Credentials | undefined>{
    user.role = 'user';
    return this.http.post<Credentials>(`${this.basedUrl}/Account`, user).toPromise();
  }

  login(user:Credentials):Promise<any>{
    return this.http.post<any>(`${this.basedUrl}/Account/login`, user).toPromise();
  }
  ...
  public get userRole():string{
    let jwtDecode = this.jwtService.decodeToken(this.getToken());
    return jwtDecode ? jwtDecode.Role : '';
  }
